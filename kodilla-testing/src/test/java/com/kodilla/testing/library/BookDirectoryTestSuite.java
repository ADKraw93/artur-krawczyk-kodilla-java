...
class BookDirectoryTestSuite {
    ...
    void testListBooksInHandsOfNoBooks() {
        // Given
        BookLibrary bookLibrary = new BookLibrary(libraryDatabaseMock);
        LibraryUser user = new LibraryUser("John", "Smith", "832123422");
        List<Book> resultList = new ArrayList<>();
        when(libraryDatabaseMock.listBooksInHandsOf(user)).thenReturn(resultList);

        // When
        List<Book> theListOfUsersBooks = bookLibrary.listBooksInHandsOf(user);

        // Then
        assertEquals(0, theListOfUsersBooks.size());
    }
