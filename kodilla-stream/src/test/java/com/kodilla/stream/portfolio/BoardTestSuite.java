...
class BoardTestSuite {
    ...
    void testAddTaskListAverageWorkingOnTask(){
        ...
        OptionalDouble theResult = IntStream.range(0, listOfTasks.size())
                .map(t -> listOfTasks.get(t).getCreated().until(LocalDate.now()).getDays()) //potrzebowałem wcześniej przygotować listę, nie wiem jak te dwa streamy upakować w jedno
                .average();
        double theResultAsDouble = theResult.orElse(0);

        //Then
        Assertions.assertEquals(10, theResultAsDouble);
    }
